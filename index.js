...
// ------------------------------------
const RESTAURANT_EMAIL = process.env.EMAIL_USER || 'samgyupmasaya@gmail.com';
const RESTAURANT_PASS = process.env.EMAIL_PASS || 'zetsngkxipsergrm'; // ⬅️ Changed to use env
...
const db = mysql.createPool({
    // ⬅️ Changed to use standard Railway variables
...
db.getConnection()
    ...
    .catch(err => {
        console.error('❌ Order Service DB Connection Error:', err);
        process.exit(1);
    });
...
// --------------------------------------------------
// ⬅️ Set inter-service URL via environment variable
...
// --------------------------------------------------
// ⬅️ Set inter-service URL via environment variable
const CUSTOMER_AUTH_URL = process.env.CUSTOMER_AUTH_URL || 'http://localhost:5004';
async function getCustomerDetails(customerId) {
    try {
        // ⬅️ Using env variable here
        const { data } = await axios.get(`${CUSTOMER_AUTH_URL}/customer/${customerId}/details`);
        return data;
    } catch (err) {
...
// --- Start Server ---
const PORT = process.env.PORT || 5003; // ⬅️ Using env.PORT
